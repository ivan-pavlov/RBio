...
# Кажется, можно взять sampsize=40
```
...
errors.nodesize <- sapply(res_df$nodesize, function(x) wrapper(train.data, train.data$Age, test.data, test.data$Age, 50, ntree=125, mtry=10, nodesize=x, replace=T, sampsize=40))
...
errors.mtry <- sapply(res_df$mtry, function(x) wrapper(train.data, train.data$Age, test.data, test.data$Age, 50, ntree=125, mtry=x, nodesize=3, replace=T, sampsize=40))
...
cross.results <- apply(cross.validation, 1, function(test.sample){
  ...
  return(wrapper(train.data, train.data$Age, test.data, test.data$Age, 50, ntree=125, mtry=2, nodesize=3, replace=T, sampsize=40))
})
...
### С подобранными параметрами работает немного лучше, неплохо
